...
class TptHook(TtuHook):
    ...
    It supports DDL operations using tbuild and data loading operations using tdload.
    It extends the `TtuHook` and integrates with Airflow's SSHHook for remote execution.

    The TPT operations are used to interact with Teradata databases for high-performance
    data loading and DDL operations such as creating, altering, or dropping tables.
        ...
        with self.preferred_temp_directory() as tmp_dir:
            ...
            try:
                ...
                with self.ssh_hook.get_conn() as ssh_client:
                    verify_tpt_utility_on_remote_host(ssh_client, "tbuild", self.log)
                    password = generate_random_password()
                    generate_encrypted_file_with_openssl(local_script_file, password, encrypted_file_path)
                    transfer_file_sftp(
                        ssh_client, encrypted_file_path, remote_encrypted_script_file, self.log
                    )
                    decrypt_remote_file(
                        ssh_client, remote_encrypted_script_file, remote_script_file, password, self.log
                    )

                    set_remote_file_permissions(ssh_client, remote_script_file, self.log)
                    ...
                    remote_secure_delete(
                        ssh_client, [remote_encrypted_script_file, remote_script_file], self.log
                    )
    ...
    def _transfer_to_and_execute_tdload_on_remote(
        ...
        try:
            ...
            with self.ssh_hook.get_conn() as ssh_client:
                ...
                transfer_file_sftp(ssh_client, encrypted_file_path, remote_encrypted_job_file, self.log)
                decrypt_remote_file(
                    ssh_client, remote_encrypted_job_file, remote_job_file, password, self.log
                )
                ...
                # Clean up remote files before checking exit status
                remote_secure_delete(ssh_client, [remote_encrypted_job_file, remote_job_file], self.log)
            ...
            raise AirflowException(
                f"An unexpected error occurred while executing TDLoad script on remote machine: {str(e)}"
            )
        finally:
            # Clean up local files
            secure_delete(encrypted_file_path, self.log)
            secure_delete(local_job_var_file, self.log)
    ...
    def _execute_tdload_locally(
        ...
        with self.preferred_temp_directory() as tmp_dir:
            ...
            # Set file permission to read-only for the current user (no permissions for group/others)
            set_local_file_permissions(local_job_var_file, self.log)
            ...
            if not shutil.which("tdload"):
                raise AirflowException("tdload binary not found in PATH.")
            ...
            try:
                ...
                if sp.returncode != 0:
                    error_msg = "\n".join(error_lines) if error_lines else "Unknown error"
                    raise AirflowException(
                        f"tdload command failed with return code {sp.returncode}: {error_msg}"
                    )
                return sp.returncode
            except Exception as e:
